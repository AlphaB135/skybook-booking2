...
export default {
  ...
  // ===============================================
  // ส่วนที่ 1: PROPS (รถพ่วง)
  // Props คือ "ของที่แม่ฝากมาให้ลูกใช้อย่างเดียว" 
  // ** ห้ามลูกแก้ไขเด็ดขาด !! **
  // ===============================================
  props: {
    // ข้อมูลผู้โดยสาร (แม่ส่ง object มาให้)
    currentUser: {
      type: Object,
      required: true
    },
    // ข้อมูลเที่ยวบิน (แม่ส่ง object มาให้)
    selectedFlight: {
      type: Object,
      required: true
    },
    // รหัสการจอง (แม่ส่งข้อความมาให้)
    bookingReference: {
      type: String,
      required: true
    },
    // เลขที่นั่ง (แม่ส่งข้อความ A12 มาให้)
    seatNumber: {
      type: String,
      required: true
    },
    // เลขทางออก (แม่ส่งข้อความ D4 มาให้)
    gateNumber: {
      type: String,
      required: true
    }
  ...
  // ===============================================
  // ส่วนที่ 2: ตัวอย่างการแก้ไข (ที่ทำไม่ได้!)
  // ===============================================
  methods: {
    // สมมติเราสร้างปุ่ม "เปลี่ยนที่นั่งเอง"
    tryToChangeSeat() {
      console.log("กำลังพยายามเปลี่ยนที่นั่ง...");

      // ❌ ผิดมหันต์! ทำแบบนี้ Vue จะด่า (Warning)
      // เพราะ Props คือ "ของของแม่" เรามีหน้าที่แค่ "ยืมมาโชว์"
      // เราไม่มีสิทธิ์แก้ต้นฉบับครับ
      
      // this.seatNumber = '99Z'; // <--- ถ้าแกะ Comment นี้ออก ระบบจะแจ้งเตือนสีแดงทันที!

      // ✅ วิธีที่ถูก: ต้องโทรไปบอกแม่ให้แก้ให้
      // this.$emit('update-seat', '99Z');
    }
  }
}
</script>
